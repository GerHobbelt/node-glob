...
t.test('a', function (t) {
  var g = glob('/b*/**', { root: path.resolve('a').replace(/\\/g, '/') }, function (er, matches) {
    t.ifError(er)
    var wanted = [
...
t.test('root=a, cwd=a/b', function (t) {
  var g = glob('/b*/**', { root: 'a', cwd: path.resolve('a/b').replace(/\\/g, '/') }, function (er, matches) {
    t.ifError(er)
...
t.test('combined with absolute option', function(t) {
  var g = glob('/b*/**', { root: path.resolve('a').replace(/\\/g, '/'), absolute: true }, function (er, matches) {
    t.ifError(er)
    t.same(matches, [ '/b', '/b/c', '/b/c/d', '/bc', '/bc/e', '/bc/e/f' ].map(function (m) {
      return path.join(path.resolve('a'), m).replace(/\\/g, '/')
    }))
...
t.test('cwdAbs when root=a, absolute=true, cwd=__dirname', function(t) {
   var g = glob('/b*/**', { root: path.resolve('a').replace(/\\/g, '/'), absolute: true, cwd: __dirname }, function (er, matches) {
    t.ifError(er)
